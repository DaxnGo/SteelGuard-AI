...
Final format after AI integration:

```json
"gradcam_image": "data:image/png;base64,<base64-encoded-png>"
```

`gradcam_image` is a Base64-encoded PNG image string, or `null` when no visualization is available. This field type (`string | null`) does not change between dummy and real AI implementations.
